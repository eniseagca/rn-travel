...
const Search = (props) => {
  const deger = props.route.params.deger
  ...
  function Ara (inputg) {
    return new Promise((resolve, reject) => {
      const a = Otel.filter(item => item.name.includes(inputg))
      if (a.length != 0) {
        setDondur(a)
        resolve('basarili')
      } else {
            ...
            <Box flex={0.8} mt="3">
                ...
                <HStack justifyContent={'space-around'} alignItems={'center'}>
                    ...
                    InputRightElement={<Pressable
                        ...
                        onPress={() => Ara(input).then((e) => console.log(e)).catch((error) => console.log(error))}>

                            <SearchIcon size={4} mr="2" color="muted.400"/></Pressable>}
                    />
                </HStack>
                {dondur.map((item) =>
